...
# ============================================================
# LOGIN SCREEN - HTML PURO COM SUBMISSÃO PARA O STREAMLIT
# ============================================================
if st.session_state.token is None:
    ...
    # Processar o formulário via POST (recebido do iframe)
    # O iframe envia os dados via fetch para um endpoint do Streamlit
    # Mas como o Streamlit não tem endpoints POST, vamos usar um truque
    ...
    # HTML do login - com redirecionamento para o próprio Streamlit via GET
    login_html = f'''
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
        ...
        (function() {{
            ...
            const STREAMLIT_URL = window.location.origin + window.location.pathname;
            ...
            document.getElementById('loginForm').addEventListener('submit', function(e) {{
                ...
                .then(function(data) {{
                    if (data.access_token) {{
                        // Redirecionar para o Streamlit com os dados no URL
                        window.location.href = STREAMLIT_URL + '?login_success=true&token=' + encodeURIComponent(data.access_token) + '&username=' + encodeURIComponent(username);
                    }} else {{
    ...
    # Verificar se há token nos query params (vindo do redirecionamento)
    if st.query_params.get("login_success") == "true":
